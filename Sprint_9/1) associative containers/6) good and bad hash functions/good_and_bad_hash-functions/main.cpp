...
struct PlateHasherAll {
    ...
    uint64_t operator()(const VehiclePlate& plate) const {
        const auto& letters = plate.GetLetters();
        uint64_t hash_plate = ((letters[0] - 'A' + 1) * 1000000) + ((letters[1] - 'A' + 1) * 100000000) + ((letters[1] - 'A' + 1) * 10000000000);
        hash_plate += plate.GetDigits() + plate.GetRegion() * 1000;
        return hash_plate;
    }
