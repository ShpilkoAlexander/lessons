...
public:
    ...
    int64_t GetCurrentBill(const VehiclePlate& car) const {
        // место для вашей реализации
        Duration duration = Clock::now() - Clock::now();
        auto car_index = complete_parks_.find(car);
        if (car_index != complete_parks_.end()) {
            duration += car_index->second;
        }

        auto car_index_now = now_parked_.find(car);
        if (car_index_now != now_parked_.end()) {
            duration += (Clock::now() - car_index_now->second);
        }

        return chrono::duration_cast<chrono::seconds>(duration).count() * cost_per_second_;
    }

    int64_t GetCurrentBillClear(const VehiclePlate& car)  {
        ...
        if (car_index != complete_parks_.end()) {
            duration += car_index->second;
            //complete_parks_.erase(car_index);
        }

        auto car_index_now = now_parked_.find(car);
        if (car_index_now != now_parked_.end()) {
            duration += (Clock::now() - car_index_now->second);
            car_index_now->second = Clock::now();
        }
    ...
    unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> EndPeriodAndGetBills() {
        ...
        int64_t bill = 0;
        for (auto& [plate, time]: complete_parks_) {
            bill = GetCurrentBillClear(plate);
            if (bill != 0){
                bills[plate] = bill;
            }
        }

        for (auto& [plate, time]: now_parked_) {
            if (time != Clock::now()) {
                bills[plate] = GetCurrentBillClear(plate);
            }
        }
