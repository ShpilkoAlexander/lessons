...
void DrawAnimals(const std::vector<const Animal*>& animals, ostream& out) {
    for (const Animal* animal : animals) {
        if (const Drawable* drawableAnimal = dynamic_cast<const Drawable*>(animal)) {
            drawableAnimal->Draw(out);
        }
    }
}
...
void TalkToAnimals(const std::vector<const Animal*> animals, ostream& out) {
    for (const Animal* animal : animals) {
        if (const Speakable* speakableAnimal = dynamic_cast<const Speakable*>(animal)) {
            speakableAnimal->Speak(out);
        }
    }
}
