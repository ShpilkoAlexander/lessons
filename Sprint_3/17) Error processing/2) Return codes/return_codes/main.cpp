...
public:
    ...
    [[nodiscard]] bool AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        ...
        //Проверка на корректность символов документа
        if(!IsValidWord(document)){
            return false;
        }

        //Проверка на корректность введенного id
        bool is_invalid_id = (document_id < 0) || documents_.count(document_id);
        if(is_invalid_id){
            return false;
        }
    ...
    [[nodiscard]] bool  FindTopDocuments(const string& raw_query, DocumentPredicate document_predicate,
        ...
        if(!IsValidQuery(query.minus_words)){
            return false;
        }
    ...
    int GetDocumentId(int serial_number){
        if( serial_number < 0 || serial_number >= GetDocumentCount()){
            return INVALID_DOCUMENT_ID;
        }
        else{
            return documents_id_[static_cast<unsigned long>(serial_number)];
        }
    }
...
private:
    ...
    static bool IsValidQuery(const set<string>& words){
        for(const string& word : words){
            if((word[0] == '-') || (word == "")){
                return false;
            }
