...
public:
    ...
    bool AreSynonyms(const string& first_word, const string& second_word) const {
        ...
        bool second_in_first = synonyms_.at(first_word).count(second_word);
        bool first_in_second = synonyms_.at(second_word).count(first_word);

        return second_in_first || first_in_second;
    }
