...
public:
    ...
    std::shared_ptr<Value> GetValue(const Key& key) {
        auto found_value = cache_.find(key);
        if (found_value != cache_.end() && !(found_value->second.expired)()) {
            return found_value->second.lock();
        }

        else {
            auto sh_ptr = value_factory_(key);
            cache_[key] = sh_ptr;
            return sh_ptr;
        }
    }
...
//    cout << "Alice: " << (alice1 == nullptr) << endl;
//    cout << "Bob: " << bob->GetId() << endl;
...
public:
    ...
    std::shared_ptr<Book> operator()(const std::string& title) const {
        auto found_book = store_.find(title);
        if (found_book != store_.end()) {
            ++load_count_;
            return std::make_shared<Book>(found_book->first, found_book->second);
        }
